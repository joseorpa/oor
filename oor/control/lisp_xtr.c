...
mc_entry_expiration_timer_cb(oor_timer_t *timer)
{
    ...
    oor_timer_t *refresh_timer;
    timer_map_req_argument *timer_arg;

    /* Check the last time we used the map cache entry */
    if (difftime(time(NULL), mcache_entry_last_req_time(mce)) > OOR_ACTIVE_MAPPING_TIMEOUT){
        OOR_LOG(LDBG_1,"Got expiration for EID %s", lisp_addr_to_char(addr));
        tr_mcache_remove_entry(xtr, mce);
    }else{
        OOR_LOG(LDBG_1,"Got expiration for EID %s. Trying to refresh mapping", lisp_addr_to_char(addr));
        timer_arg = timer_map_req_arg_new_init(mce,mcache_entry_requester_eid(mce));
        refresh_timer = oor_timer_with_nonce_new(MAP_REQUEST_RETRY_TIMER,xtr,send_map_request_retry_cb,
                timer_arg,(oor_timer_del_cb_arg_fn)timer_map_req_arg_free);
        send_map_request_retry_cb(refresh_timer);
    }
    return(GOOD);
}
...
tr_recv_map_reply(lisp_xtr_t *xtr, lbuf_t *buf, uconn_t *udp_con)
{
    ...
    if (!MREP_RLOC_PROBE(mrep_hdr)){
        ...
        mce = t_mr_arg->mce;
        src_eid = lisp_addr_clone(mcache_entry_requester_eid(mce));
        ...
        for (i = 0; i < records; i++) {
            ...
            if (!active_entry) {
                ...
                mcache_entry_set_requester_eid(mce,src_eid);
            } else {
...
tr_get_fwd_entry(lisp_xtr_t *xtr, packet_tuple_t *tuple)
{
    ...
    /* Update time of the last time the mce has been used in the data plane */
    mcache_entry_set_last_req_time(mce,time(NULL));
